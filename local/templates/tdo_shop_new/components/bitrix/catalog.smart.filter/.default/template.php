...
<div id="sort-filter-js" class="sort_filter smart-filter mb-4 <?=$templateData["TEMPLATE_CLASS"]?> <? if($arParams["FILTER_VIEW_MODE"] == "HORIZONTAL")
	echo "smart-filter-horizontal" ?>">

	<div class="sort-heading">
		<h3 class="text-small-h sort-heading__title">Фильтр</h3>
		<span class="sort-heading__close" id="sort-close">
			<svg xmlns="http://www.w3.org/2000/svg" version="1" viewBox="0 0 24 24"><path d="M13 12l5-5-1-1-5 5-5-5-1 1 5 5-5 5 1 1 5-5 5 5 1-1z"></path></svg>
		</span>
	</div>

	<div class="sort-selected">
		<div class="sort-selected_top">
			<div class="sort-selected_top-text">Выбранные фильтры</div>
			<a class="sort-selected_top-reset js-del_filter" href="#">Сбросить всё</a>
		</div>
		<div class="sort-selected_curent js-sort-selected"></div>
	</div>
		...
		<form name="<? echo $arResult["FILTER_NAME"] . "_form" ?>" action="<? echo $arResult["FORM_ACTION"] ?>"
			...
			<div class="row">
				<? foreach ($arResult["ITEMS"] as $key => $arItem)//prices
				{
					...
					if(isset($arItem["PRICE"])):
						...
						else
						{
							...
							for ($i = 0; $i < $step_num; $i++)
							{
								$prices[ $i ] = number_format($arItem["VALUES"]["MIN"]["VALUE"] + $step * $i,
								                              $precision, ".", "");
							}
						...
						<div class="<? if($arParams["FILTER_VIEW_MODE"] == "HORIZONTAL"): ?>col-sm-6 col-md-4<? else: ?>col-12<? endif ?> mb-2 smart-filter-parameters-box bx-active">
							...
							<div class="smart-filter-parameters-box-title" onclick="smartFilter.hideFilterProps(this)">
								<span class="smart-filter-parameters-box-title-text sort_filter-head">Цена, руб.</span>
							...
							<div class="smart-filter-block" data-role="bx_filter_block">
								<div class="smart-filter-parameters-box-container">
									<div class="smart-filter-input-group-number">
										<div class="d-flex justify-content-safe sort_filt-price">
											<div class="form-group" style="width: calc(50% - 10px);">
												<div class="smart-filter-input-container">
													<span>от</span>
													<input
															class="min-price form-control form-control-sm sort_filt-price_input"
															type="number"
															name="<? echo $arItem["VALUES"]["MIN"]["CONTROL_NAME"] ?>"
															id="<? echo $arItem["VALUES"]["MIN"]["CONTROL_ID"] ?>"
															value="<? echo $arItem["VALUES"]["MIN"]["HTML_VALUE"] ?>"
															onkeyup="smartFilter.keyup(this)"
															placeholder="<?=intval($arItem["VALUES"]["MIN"]["FILTERED_VALUE"]) ? $arItem["VALUES"]["MIN"]["FILTERED_VALUE"] : $arItem["VALUES"]["MIN"]["VALUE"]?> руб."
													/>
												</div>
											</div>
											...
											<div class="form-group" style="width: calc(50% - 10px);">
												<div class="smart-filter-input-container">
													<span>до</span>
													<input
															class="max-price form-control form-control-sm sort_filt-price_input"
															type="number"
															name="<? echo $arItem["VALUES"]["MAX"]["CONTROL_NAME"] ?>"
															id="<? echo $arItem["VALUES"]["MAX"]["CONTROL_ID"] ?>"
															value="<? echo $arItem["VALUES"]["MAX"]["HTML_VALUE"] ?>"
															onkeyup="smartFilter.keyup(this)"
															placeholder="<?=intval($arItem["VALUES"]["MAX"]["FILTERED_VALUE"]) ? $arItem["VALUES"]["MAX"]["FILTERED_VALUE"] : $arItem["VALUES"]["MAX"]["VALUE"]?> руб."
													/>
												</div>
											</div>
										</div>
				...
				foreach ($arResult["ITEMS"] as $key => $arItem)
				{
					...
					<div class="<? if($arParams["FILTER_VIEW_MODE"] == "HORIZONTAL"): ?>col-sm-6 col-md-4<? else: ?>col-lg-12<? endif ?> mb-2 smart-filter-parameters-box <? if($arItem["DISPLAY_EXPANDED"] == "Y"): ?>bx-active<? endif ?> sort_filter-item">


						<div class="smart-filter-parameters-box-title " onclick="smartFilter.hideFilterProps(this)">

							<span class="smart-filter-parameters-box-title-text sort_filter-head"><?=$arItem["NAME"]?></span>

							<span data-role="prop_angle"
							      class="smart-filter-angle smart-filter-angle-<? if($arItem["DISPLAY_EXPANDED"] == "Y"): ?>up<? else: ?>down<? endif ?>">
								<span class="smart-filter-angles"></span>
							</span>
						...
						<div class="smart-filter-block" data-role="bx_filter_block">
							<div class="smart-filter-parameters-box-container">
									...
									<div class="smart-filter-input-group-number">
										...
										<div class="smart-filter-slider-track-container">
											<div class="smart-filter-slider-track" id="drag_track_<?=$key?>">
												...
												$value1    = number_format($arItem["VALUES"]["MIN"]["VALUE"],
												                           $precision, ".", "");
												$value2    = number_format($arItem["VALUES"]["MIN"]["VALUE"] + $step,
												                           $precision, ".", "");
												$value3    = number_format($arItem["VALUES"]["MIN"]["VALUE"] + $step * 2,
												                           $precision, ".", "");
												$value4    = number_format($arItem["VALUES"]["MIN"]["VALUE"] + $step * 3,
												                           $precision, ".", "");
												$value5    = number_format($arItem["VALUES"]["MAX"]["VALUE"],
												                           $precision, ".", "");
												?>
												<div class="smart-filter-slider-ruler p1"><span><?=$value1?></span>
												</div>
												<div class="smart-filter-slider-ruler p2"><span><?=$value2?></span>
												</div>
												<div class="smart-filter-slider-ruler p3"><span><?=$value3?></span>
												</div>
												<div class="smart-filter-slider-ruler p4"><span><?=$value4?></span>
												</div>
												<div class="smart-filter-slider-ruler p5"><span><?=$value5?></span>
												</div>
								...
								default:
								?>
									<div class="smart-filter-input-group-checkbox-list sort_filter-check_wrap">
										<? foreach ($arItem["VALUES"] as $val => $ar): ?>
											<div class="form-group form-check mb-1">
												<input
														...
														id="<? echo $ar["CONTROL_ID"] ?>"
														class="form-check-input checkbox-custom-hidd"
													<? echo $ar["CHECKED"] ? 'checked="checked"' : '' ?>
												...
												<label data-role="label_<?=$ar["CONTROL_ID"]?>"
												       class="sort_filter-label-text smart-filter-checkbox-text form-check-label"
												       for="<? echo $ar["CONTROL_ID"] ?>">
													<?=$ar["VALUE"];
													if($arParams["DISPLAY_ELEMENT_COUNT"] !== "N" && isset($ar["ELEMENT_COUNT"])):
														?>&nbsp;(<span
									...
									if(count($arItem["VALUES"]) > 10)
								{
									...
									<a href="javascript:void(0);"
									   class="smart-filter-input-group-checkbox-list-toggle js-smart-filter-inputs-toggle">Посмотреть
										все</a>
									<?
								}
									//endregion
			...
			<div class="row">
				<div class="col smart-filter-button-box">
					<div class="smart-filter-block">
						<div class="mb-2">
							<input
									class="btn-filter js-btn-filter-apply"
									type="submit"
									id="set_filter"
									name="set_filter"
						...
						<div class="mb-2">
							...
							/>
							<div class="smart-filter-popup-result <? if($arParams["FILTER_VIEW_MODE"] == "VERTICAL")
...
<script type="text/javascript">
    ...
    jQuery(document).on('click', '.js-del_filter', function (e) {
        e.preventDefault();

        jQuery('#del_filter').click();
    });

    jQuery(document).ready(function ($) {
        jQuery(document).trigger('updSelFilters');
    });

    jQuery(document).on('updSelFilters', function (event) {
        let $selectItems = jQuery('.smart-filter-section .checkbox-custom-hidd:checked')
        let selWrap = jQuery('.js-sort-selected')

        selWrap.html('');

        jQuery.each($selectItems, function (i, val) {
            let $el = $selectItems.eq(i)
            let textLabel = $el.siblings('.sort_filter-label-text').text()
            let fitem = $el.attr('id')

            selWrap.append('<div class="sort-selected_curent_item" data-fitem="' + fitem + '">' + textLabel + '</div>')
        });
    });

    jQuery(document).on('click', '.sort-selected_curent_item', function (e) {
        e.preventDefault();

        let fitem = jQuery(this).data('fitem')
        jQuery('#' + fitem).click()
    });

    jQuery(document).on('change', '.checkbox-custom-hidd', function (e) {
        jQuery(document).trigger('updSelFilters');
    });
