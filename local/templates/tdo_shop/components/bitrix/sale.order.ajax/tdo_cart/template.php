...
?>
	<div id="basket-root" class="bx-basket bx-<?=$arParams['TEMPLATE_THEME']?> bx-step-opacity">

		<div class="cart-content" id="bx-soa-order">
			...
			<form action="<?=POST_FORM_ACTION_URI?>" method="POST" name="ORDER_FORM"
			      class="bx-soa-wrapper mb-4<?=$themeClass?>" id="bx-soa-order-form"
				...
				<div class="cart-bottom">
					<div class="form-gray">
						...
						<div id="bx-soa-order_old" class="row">
							<!--	MAIN BLOCK	-->
							<div class="col-12 bx-soa">
								...
								<? if($arParams['DELIVERY_TO_PAYSYSTEM'] === 'p2d'): ?>
									...
									<!--	PICKUP BLOCK	-->
									<div id="bx-soa-pickup" data-visited="false" class="bx-soa-section"
									     style="display:none">
										<div class="bx-soa-section-title-container d-flex justify-content-between align-items-center flex-nowrap">
											<div class="bx-soa-section-title" data-entity="section-title">
												<span class="bx-soa-section-title-count"></span>
					...
					<div class="cart-bottom_right">
						<!--	SIDEBAR BLOCK	-->
						<div id="bx-soa-total" class="cart-bottom_right-content bx-soa-coupon">
							<div class="bx-soa-coupon-item-wrap"></div>

							<div id="bx-soa-coupon" class="sale-kod form">
								<input placeholder="У меня есть промокод" type="text" class="sale-kod_input"
								       data-entity="basket-coupon-input">
								<button type="button" class="basket-coupon-block-coupon-btn sale-kod_btn">Применить
								</button>
							</div>
			...
			?>
            <div id="bx-soa-region-modern" data-visited="true" class="bx-soa-section bx-active d-none">
                <select id="CITY_SELECT" name="ORDER_PROP_44" type="location" class="region-select js-region-select">
					<?
					?>
					...
					while ($item = $res->fetch()) {
						?>
                        <option value="<?=$item['SALE_LOCATION_LOCATION_CHILDREN_CODE']?>"><?=$item['NAME_RU']?></option>
						<?
					}
					?>
                </select>
...
*/
	...
	<div id="bx-soa-soc-auth-services" style="display:none">
		<?
		$arServices                              = false;
		$arResult['ALLOW_SOCSERV_AUTHORIZATION'] = Main\Config\Option::get('main', 'allow_socserv_authorization',
		                                                                   'Y') != 'N' ? 'Y' : 'N';
...
?>
	<script>
        ...
        /*
					BX.Sale.OrderAjaxComponentExt.editActiveBasketBlock(true);
					BX.Sale.OrderAjaxComponentExt.editActiveRegionBlock(true);
					BX.Sale.OrderAjaxComponentExt.editActiveDeliveryBlock(true);
					BX.Sale.OrderAjaxComponentExt.editActivePaySystemBlock(true);
					BX.Sale.OrderAjaxComponentExt.editActivePickUpBlock(true);
					BX.Sale.OrderAjaxComponentExt.editActivePropsBlock(true);*/

        /*        var timeout_id = setTimeout(function(){

					BX.Sale.OrderAjaxComponentExt.editRegionBlock(true);
					BX.Sale.OrderAjaxComponentExt.editDeliveryBlock(true);
					BX.Sale.OrderAjaxComponentExt.editPaySystemBlock(true);
					BX.Sale.OrderAjaxComponentExt.editPickUpBlock(true);
					// BX.Sale.OrderAjaxComponentExt.editPropsBlock(true);
				}, 100)*/
        ...
        // jQuery(document).on('click', '.cart-content-count_plus, .cart-content-count_min', function (e) {
            // var timeout_id = setTimeout(function () {
	...
	<script>
        ...
        BX.saleOrderAjax.init(<?=CUtil::PhpToJSObject(array(
			                                              ...
			                                              'messages'   => array(
				                                              ...
				                                              'moreInfoLocation' => '--- ' . Loc::getMessage('SOA_NOT_SELECTED_ALT'),
				                                              // spike: for children of cities we place this prompt
