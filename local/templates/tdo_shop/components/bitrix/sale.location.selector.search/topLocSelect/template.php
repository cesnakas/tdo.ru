...
?>
		...
		<? if(is_array($arResult['DEFAULT_LOCATIONS']) && !empty($arResult['DEFAULT_LOCATIONS'])): ?>
            <div class="wrap-select-cities">
				<? foreach ($arResult['DEFAULT_LOCATIONS'] as $lid => $loc): ?>
                    <div class="select-sity-wraps">
                        <a href="javascript:void(0)"
                           data-id="<?=intval($loc['ID'])?>"
                           class="select-sity-item"
                        ><?=htmlspecialcharsbx($loc['NAME'])?></a>
                    </div>
				<? endforeach ?>
            </div>
        ...
        <div class="wrap-big-btn">
            <a href="#" class="btn big-btn btn-red">Подтвердить</a>
        </div>
// */
    ...
    <div id="sls-<?=$arResult['RANDOM_TAG']?>" class="bx-sls <? if(strlen($arResult['MODE_CLASSES'])): ?> <?=$arResult['MODE_CLASSES']?><? endif ?>">
        ...
        <form class="" action="/">
			...
			<? if(is_array($arResult['DEFAULT_LOCATIONS']) && !empty($arResult['DEFAULT_LOCATIONS'])): ?>
                ...
                <div class="wrap-select-cities quick-locations">
					<? foreach ($arResult['DEFAULT_LOCATIONS'] as $lid => $loc): ?>
                        <div class="select-sity-wrap">
                            <a href="javascript:void(0)" data-id="<?=intval($loc['ID'])?>" class="select-sity-item"><?=htmlspecialcharsbx($loc['NAME'])?></a>
                        </div>
					<? endforeach ?>
                </div>
            ...
            <div class="wrap-big-btn">
                <a href="#" class="btn big-btn btn-red js-location-set">Подтвердить</a>
            </div>
